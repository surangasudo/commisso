...
export const PrintableReceipt = React.forwardRef<HTMLDivElement, PrintableReceiptProps>(({ sale, products }, ref) => {
    ...
    // This function renders the content only when there's a sale,
    // inside the stable parent div that holds the ref.
    const renderContent = () => {
        if (!sale) return null;

        const subtotal = sale.items.reduce((acc, item) => acc + (item.quantity * item.unitPrice), 0);
        const discount = subtotal + (sale.taxAmount || 0) - sale.totalAmount;
